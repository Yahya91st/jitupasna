...
class Format2Form4 extends Model
{
    ...
    public function calculateTotalKerusakan()
    {
        ...
        // Perhitungan kerusakan bangunan pendidikan
        foreach ($bangunan as $b) {
            $totalKerusakan += (($this->{$b.'_berat_negeri'} ?? 0) + ($this->{$b.'_berat_swasta'} ?? 0)) * ($this->{$b.'_harga_bangunan'} ?? 0);
            $totalKerusakan += (($this->{$b.'_sedang_negeri'} ?? 0) + ($this->{$b.'_sedang_swasta'} ?? 0)) * ($this->{$b.'_harga_bangunan'} ?? 0);
            $totalKerusakan += (($this->{$b.'_ringan_negeri'} ?? 0) + ($this->{$b.'_ringan_swasta'} ?? 0)) * ($this->{$b.'_harga_bangunan'} ?? 0);
        }
        
        // Biaya tenaga kerja dan alat berat (dipindahkan dari kerugian ke kerusakan)
        ...
        // Biaya sekolah sementara (dipindahkan dari kerugian ke kerusakan)
        $totalKerusakan += ($this->jumlah_sekolah_sementara ?? 0) * ($this->harga_sekolah_sementara ?? 0);
