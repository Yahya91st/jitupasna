...
class Format3Controller extends Controller
{
    ...
    {
        $formKesehatan = Format3Form4::with(['bencana', 'bencana.desa'])->findOrFail($id);
        $bencana = $formKesehatan->bencana;
        
        // Convert Format3Form4 data to HealthReport-like structure for the view
        $healthReports = [];
        
        // Define facility types and their field prefixes
        $facilityTypes = [
            'rumah_sakit' => 'rs',
            'puskesmas' => 'puskesmas', 
            'poliklinik' => 'poliklinik',
            'puskesmas_pembantu' => 'pustu',
            'polindes' => 'polindes',
            'posyandu' => 'posyandu'
        ];
        
        foreach ($facilityTypes as $facilityName => $prefix) {
            // Check if facility has any data
            $hasData = false;
            $facilityData = [];
            
            // Check for government (negeri) facility
            $rbNegeri = $formKesehatan->{$prefix . '_rb_negeri'} ?? 0;
            $rsNegeri = $formKesehatan->{$prefix . '_rs_negeri'} ?? 0; 
            $rrNegeri = $formKesehatan->{$prefix . '_rr_negeri'} ?? 0;
            
            if ($rbNegeri > 0 || $rsNegeri > 0 || $rrNegeri > 0) {
                $hasData = true;
                $facilityData[] = (object) [
                    'nama_fasilitas' => ucwords(str_replace('_', ' ', $facilityName)) . ' (Negeri)',
                    'rusak_berat' => $rbNegeri,
                    'rusak_sedang' => $rsNegeri,
                    'rusak_ringan' => $rrNegeri,
                    'biaya_rb' => $formKesehatan->{$prefix . '_harga_bangunan'} ?? 0,
                    'biaya_rs' => $formKesehatan->{$prefix . '_harga_obat'} ?? 0,
                    'biaya_rr' => $formKesehatan->{$prefix . '_harga_meubelair'} ?? 0,
                    'total_biaya' => ($formKesehatan->{$prefix . '_harga_bangunan'} ?? 0) + 
                                   ($formKesehatan->{$prefix . '_harga_obat'} ?? 0) + 
                                   ($formKesehatan->{$prefix . '_harga_meubelair'} ?? 0),
                    'keterangan' => '-'
                ];
            }
            
            // Check for private (swasta) facility  
            $rbSwasta = $formKesehatan->{$prefix . '_rb_swasta'} ?? 0;
            $rsSwasta = $formKesehatan->{$prefix . '_rs_swasta'} ?? 0;
            $rrSwasta = $formKesehatan->{$prefix . '_rr_swasta'} ?? 0;
            
            if ($rbSwasta > 0 || $rsSwasta > 0 || $rrSwasta > 0) {
                $hasData = true;
                $facilityData[] = (object) [
                    'nama_fasilitas' => ucwords(str_replace('_', ' ', $facilityName)) . ' (Swasta)',
                    'rusak_berat' => $rbSwasta,
                    'rusak_sedang' => $rsSwasta, 
                    'rusak_ringan' => $rrSwasta,
                    'biaya_rb' => $formKesehatan->{$prefix . '_harga_peralatan'} ?? 0,
                    'biaya_rs' => 0,
                    'biaya_rr' => 0,
                    'total_biaya' => $formKesehatan->{$prefix . '_harga_peralatan'} ?? 0,
                    'keterangan' => '-'
                ];
            }
            
            if ($hasData) {
                $healthReports[$facilityName] = $facilityData;
            }
        }
        
        return view('forms.form4.format3.show-format3', compact('formKesehatan', 'bencana', 'healthReports'));
    }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            return redirect()->route('forms.form4.list-format3', ['bencana_id' => $data['bencana_id']])
