...
Route::prefix('/forms')->middleware(['auth', 'verified'])->name('forms.')->group(function () {
    ...
    Route::prefix('form4')->name('form4.')->group(function() {
        ...
        Route::get('/format1form4', function() {
            return view('forms.form4.format1form4');
        })->name('format1form4');
        Route::post('/store', function() {
            // Logic to store form data - we'll just redirect for now
            return redirect()->route('forms.form4.index')->with('success', 'Data berhasil disimpan');
        })->name('store');
    });
});
