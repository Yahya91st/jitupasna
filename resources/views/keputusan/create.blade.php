...
@section('content')
                        ...
                        <tbody>
                            ...
                            @forelse($summaries as $index => $summary)
                                <tr>
                                    ...
                                    <td class="text-end">
                                        Rp {{ number_format($summary['total_kerusakan'], 0, ',', '.') }}
                                    </td>
        ...
        <div class="main-card">
            ...
            @if ($keputusan)
                <form action="{{ route('keputusan.update', $keputusan) }}" method="POST">
                    ...
                    @method('PUT')
                @else
                    <form action="{{ route('keputusan.store', $laporan) }}" method="POST">
            @endif
            ...
            <div class="mb-3">
                ...
                <select name="prioritas" class="form-select">
                    ...
                    <option value="sangat_tinggi" {{ old('prioritas', $keputusan->prioritas ?? '') == 'sangat_tinggi' ? 'selected' : '' }}>
                        Sangat Tinggi
                    </option>

                    <option value="tinggi" {{ old('prioritas', $keputusan->prioritas ?? '') == 'tinggi' ? 'selected' : '' }}>
                        Tinggi
                    </option>

                    <option value="sedang" {{ old('prioritas', $keputusan->prioritas ?? '') == 'sedang' ? 'selected' : '' }}>
                        Sedang
                    </option>

                    <option value="rendah" {{ old('prioritas', $keputusan->prioritas ?? '') == 'rendah' ? 'selected' : '' }}>
                        Rendah
                    </option>
            ...
            <div class="mb-3">
                ...
                <textarea name="keputusan" class="form-control" rows="5" placeholder="Tuliskan keputusan pimpinan">{{ old('keputusan', $keputusan->keputusan ?? '') }}</textarea>
    ...
    <script>
        const labels = @json(collect($summaries)->pluck('nama_kampung'));

        const kerusakan = @json(collect($summaries)->pluck('total_kerusakan'));

        const kerugian = @json(collect($summaries)->pluck('total_kerugian'));
    </script>
