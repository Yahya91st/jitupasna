        ...
        <table class="table table-hover table-lg">
            ...
            <tbody>
                @foreach ($reports as $report)
                    <tr>
                        <td>{{ $report->id }}</td>
                        <td>{{ collect($report->laporan->bencana->villages)->pluck('name')->implode(', ') }}</td>
                        <td>{{ $report->nama_kampung }}</td>
                        <td>{{ $report->nama_distrik }}</td>

                        <td>

                            <a href="{{ route('forms.form4.format1.show', $report->id) }}" class="btn btn-info btn-sm">
                                Lihat
                            </a>

                            <a href="{{ route('forms.form4.format1.edit', $report->id) }}" class="btn btn-warning btn-sm">
                                Edit
                            </a>
