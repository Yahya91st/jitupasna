            ...
            <div class="table-responsive">
                <table class="table table-bordered mb-4">
                    ...
                    <tbody>
                        @php
                            $slug_format6 = [
                                1 => 'Struktur Pengambilan Air',
                                2 => 'Instalasi Pemurnian Air',
                                3 => 'Sistem Perpipaan',
                                4 => 'Sistem Penyimpanan',
                                5 => 'Sumur',
                                6 => 'Lain',
                                7 => 'Jaringan Pembuangan',
                                8 => 'Septik Tank',
                                9 => 'Sistem Pengumpulan Limbah Padat',
                                10 => 'WC Umum',
                            ];

                            $groups_format6 = [
                                [
                                    'Sarana dan Prasarana Air Minum' => 'struktur_air',
                                    'instalasi_pemurnian',
                                    'perpipaan',
                                    'penyimpanan',
                                    'sumur',
                        ...
                        @endphp
                        @foreach ([['Struktur Pengambilan Air', 'struktur_air'], ['Instalasi Pemurnian Air', 'instalasi_pemurnian'], ['Sistem Perpipaan', 'perpipaan'], ['Sistem Penyimpanan', 'penyimpanan'], ['Sumur', 'sumur'], ['Lain-lain', 'lain'], ['Jaringan Pembuangan', 'sanitasi'], ['Septik Tank', 'septik'], ['Sistem Pengumpulan Limbah Padat', 'limbah_padat'], ['WC Umum', 'wc_umum']] as [$label, $prefix])
                            <tr>
                                <td></td>
                                <td class="align-middle fw-bold">{{ $label }}</td>
                                <td><input type="number" name="{{ $prefix }}_unit" class="form-control" min="0" value="{{ old($prefix . '_rb_negeri', $data->{$prefix . '_rb_negeri'} ?? '0') }}"></td>
                                <td><input type="number" name="{{ $prefix }}_jumlah" class="form-control" min="0" value="{{ old($prefix . '_rb_swasta', $data->{$prefix . '_rb_swasta'} ?? '0') }}"></td>
                                <td><input type="number" name="{{ $prefix }}_harga_satuan" class="form-control" min="0" value="{{ old($prefix . '_rs_negeri', $data->{$prefix . '_rs_negeri'} ?? '0') }}"></td>
                                <td><input type="text" name="{{ $prefix }}_keterangan" class="form-control" value="{{ old($prefix . '_rs_swasta', $data->{$prefix . '_rs_swasta'} ?? ' ') }}"></td>
                            </tr>
                        @endforeach
