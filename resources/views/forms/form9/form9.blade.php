...
@section('content')
    ...
    <div class="container">
        ...
        <form action="{{ route('forms.form9.store') }}" method="POST">
                    ...
                    <tbody>
                        @php
                            $rows = [
                                ...
                                ['no' => 'j', 'pertanyaan' => 'Jumlah anak balita', 'jawaban' => ['1', '2', '3', '>3']],
                                ['no' => 'k', 'pertanyaan' => 'Tipe hunian sekarang', 'jawaban' => ['Rumah tinggal sendiri', 'Rumah tumpangan', 'Pengungsian', 'Fasilitas umum', 'Lain-lain']],
                                ...
                                ['no' => '25', 'pertanyaan' => 'Penghasilan keluarga setiap bulan (sebelum bencana):', 'jawaban' => ['< Rp 500.000', 'Rp 500.000 – Rp 1.500.000', 'Rp 1.500.000 – Rp 2.500.000', '> Rp 2.500.000']],
                            ];
                        @endphp
                        ...
                        @foreach ($rows as $row)
                            @foreach ($row['jawaban'] as $index => $jawaban)
                                <tr>
                                    ...
                                    <td style="padding: 5px;"><input type="number" id="jawaban_{{ $row['no'] }}_{{ $index }}_1" name="jawaban[{{ $row['no'] }}][{{ $index }}][1]" class="form-control form-control-sm"></td>
                                    <td style="padding: 5px;"><input type="number" id="jawaban_{{ $row['no'] }}_{{ $index }}_2" name="jawaban[{{ $row['no'] }}][{{ $index }}][2]" class="form-control form-control-sm"></td>
                                    <td style="padding: 5px;"><input type="number" id="jawaban_{{ $row['no'] }}_{{ $index }}_3" name="jawaban[{{ $row['no'] }}][{{ $index }}][3]" class="form-control form-control-sm"></td>
                                    <td style="padding: 5px;"><input type="number" id="jawaban_{{ $row['no'] }}_{{ $index }}_4" name="jawaban[{{ $row['no'] }}][{{ $index }}][4]" class="form-control form-control-sm"></td>
                                    <td style="padding: 5px;"><input type="number" id="jawaban_{{ $row['no'] }}_{{ $index }}_5" name="jawaban[{{ $row['no'] }}][{{ $index }}][5]" class="form-control form-control-sm"></td>
                                    <td style="padding: 5px;"><input type="number" id="jawaban_{{ $row['no'] }}_{{ $index }}_6" name="jawaban[{{ $row['no'] }}][{{ $index }}][6]" class="form-control form-control-sm"></td>
            ...
            <div class="d-flex gap-2 justify-content-center mt-4 mb-3">
                ...
                <button type="button" class="btn btn-info form-button" onclick="autoFill()">
                    <i class="bi bi-magic"></i> Auto Isi
                </button>
    ...
    `);
        ...
        function autoFill() {
            if (!confirm('Auto isi form untuk debugging?')) return;

            // isi jawaban (0/1) untuk setiap input jawaban[no][index][k]
            const jawabanInputs = document.querySelectorAll('input[name^="jawaban"]');
            jawabanInputs.forEach(inp => {
                // 0 atau 1 acak (ubah logika jika mau deterministik)
                inp.value = Math.random() < 0.5 ? '0' : '1';
            });

            // hitung jumlah per option (sum kuesioner 1..6) dan simpan ke jumlah[no][index]
            const sums = {}; // key: no|index => sum
            jawabanInputs.forEach(inp => {
                const m = inp.name.match(/^jawaban\[(.+?)\]\[(\d+)\]\[(\d+)\]$/);
                if (!m) return;
                const no = m[1],
                    idx = m[2];
                const key = no + '|' + idx;
            ...
            // isi input jumlah[...] dan kumpulkan total per question (no)
            const totalsPerNo = {}; // total per question no
            for (const key in sums) {
                const [no, idx] = key.split('|');
                const jumlahVal = sums[key];
                const jumlahEl = document.querySelector('input[name="jumlah[' + no + '][' + idx + ']"]');
                if (jumlahEl) jumlahEl.value = jumlahVal;
                totalsPerNo[no] = (totalsPerNo[no] || 0) + jumlahVal;
            }

            // isi persentase per option: (jumlah / totalPerNo) * 100
            for (const key in sums) {
                const [no, idx] = key.split('|');
                const jumlahVal = sums[key];
                const total = totalsPerNo[no] || 0;
                const percEl = document.querySelector('input[name="persentase[' + no + '][' + idx + ']"]');
                if (percEl) {
                    const perc = total ? (jumlahVal / total * 100) : 0;
                    percEl.value = perc.toFixed(1); // 1 desimal
                }
            }

            // opsional: beri highlight singkat pada sel yang diisi
            document.querySelectorAll('input').forEach(i => {
                i.style.transition = 'background-color 0.3s';
